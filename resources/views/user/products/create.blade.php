    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0 animate-fade-in">
            <div>
                ...
                <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Deploy <span
                        class="text-gradient">Asset</span></h1>
                <p class="mt-2 text-slate-500 dark:text-slate-400 font-medium tracking-wide uppercase text-[10px]">
                    Initialize a new infrastructure entry in your personal repository</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        <form action="{{ route('user.my-products.store') }}" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="space-y-6">
                                    <div>
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name') }}"
                                            class="input-field" placeholder="e.g. Premium Subscription Model" required>
                                        @error('name') <p
                                            class="mt-2 text-xs font-bold text-rose-500 uppercase tracking-tight">
                                        {{ $message }}</p> @enderror
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="description" id="description" rows="5" class="input-field"
                                            placeholder="Provide a comprehensive description of the asset's function and value..."></textarea>
                                        @error('description') <p
                                            class="mt-2 text-xs font-bold text-rose-500 uppercase tracking-tight">
                                        {{ $message }}</p> @enderror
                                    </div>
                            ...
                            <div class="pt-8 border-t border-slate-100 dark:border-slate-800">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <label for="price"
                                            class="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Unit
                                            Price (IDR)</label>
                                        <div class="relative">
                                            <span
                                    ...
                                    <div>
                                        <label for="stock"
                                            class="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Initial
                                            Capacity</label>
                                        <input type="number" name="stock" id="stock" value="{{ old('stock') }}"
                                            class="input-field" placeholder="Total units available" min="0" required>
                                        @error('stock') <p
                                            class="mt-2 text-xs font-bold text-rose-500 uppercase tracking-tight">
                                        {{ $message }}</p> @enderror
                                    </div>
                ...
                <div class="space-y-8">
                    <div class="card p-8">
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-6 flex items-center">
                            <span
                                class="w-8 h-8 rounded-lg bg-orange-500/10 text-orange-600 flex items-center justify-center mr-3">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="relative group">
                            <div id="image-preview-container"
                                class="hidden aspect-square rounded-2xl overflow-hidden border-2 border-slate-100 dark:border-slate-800 mb-6 group relative">
                            ...
                            <div id="drop-zone"
                                ...
                                <h4
                                    class="text-sm font-bold text-slate-700 dark:text-white uppercase tracking-widest mb-2">
                                    Upload Visual</h4>
                                <p class="text-[10px] text-slate-500 font-medium leading-relaxed">Drag and drop
                                    high-resolution asset imagery</p>
                                <input type="file" form="product-form" name="image" id="image" class="sr-only"
                    ...
                    <div class="card p-8 bg-slate-900">
                        <h4 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Inventory Policies
                        </h4>
                        <div class="space-y-4">
                            @foreach(['Images must show actual product', 'Prices subject to platform fee', 'Listing active upon commitment', 'Stock level determines visibility'] as $rule)
