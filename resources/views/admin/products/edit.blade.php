...
@section('content')
    ...
    <div class="mb-10 animate-fade-in">
        ...
        <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit <span
        ...
        <p class="mt-2 text-slate-500 dark:text-slate-400 font-medium tracking-wide uppercase text-[10px]">Updating
            infrastructure for item #{{ str_pad($product->id, 6, '0', STR_PAD_LEFT) }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}"
                                    class="input-field" placeholder="e.g. MacBook Pro M3 Max" required>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="5" class="input-field"
                                    placeholder="Provide a comprehensive description of the product features and capabilities...">{{ old('description', $product->description) }}</textarea>
                                @error('description') <p
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div>
                                ...
                                <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}"
                                    class="input-field" placeholder="Total units in stock" min="0" required>
        ...
        <div class="space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    @if($product->image)
                        <div id="current-image-preview"
                            class="aspect-square rounded-2xl overflow-hidden border-2 border-slate-100 dark:border-slate-800 mb-6 group relative">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="Current Product Asset"
                                class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                            <div class="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
                                <p class="text-[10px] font-black text-white uppercase tracking-widest text-center">Currently
                                    Deployed Asset</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="relative group">
                        ...
                        <div id="drop-zone"
                            ...
                            <h4 class="text-sm font-bold text-slate-700 dark:text-white uppercase tracking-widest mb-2">
                                Update Asset</h4>
                            <p class="text-[10px] text-slate-500 font-medium leading-relaxed">Drag and drop to override
                                current imagery</p>
            ...
            <div class="card p-8 bg-slate-900">
                <h4 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Modification Rules</h4>
                <div class="space-y-4">
                    @foreach(['Image overrides current asset', 'System versioning will be applied', 'Changes are reflected immediately', 'Ensure pricing is market-standard'] as $rule)
        ...
        function previewImage(event) {
            ...
            if (file) {
                ...
                reader.onload = function (e) {
                    ...
                    if (currentPreview) {
                        currentPreview.classList.add('opacity-30', 'grayscale', 'scale-90');
                        currentPreview.classList.remove('group');
                    }
                }
